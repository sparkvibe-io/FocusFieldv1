...
class PermissionDialogs {
  static Future<void> showMicrophoneRationale(BuildContext context, WidgetRef ref) async {
  ...
  static Future<void> showMicrophoneSettings(BuildContext context, WidgetRef ref) async {
    if (!context.mounted) return;
    await showDialog(
      context: context,
      barrierDismissible: true,
      builder: (ctx) => AlertDialog(
        title: const Text(PermissionConstants.microphoneSettingsDialogTitle),
        content: const Text(PermissionConstants.microphoneSettingsDialogBody),
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.of(ctx).pop();
              final detector = ref.read(silenceDetectorProvider);
              try { await detector.openSettings(); } catch (_) {}
            },
