...
class _AnimatedPulseOverlayState extends State<AnimatedPulseOverlay>
...
class _MistOverlayState extends State<MistOverlay>
    with SingleTickerProviderStateMixin {
  late final AnimationController _c;
  @override
  void initState() {
...
class ParticleDriftOverlay extends StatefulWidget {
  ...
  @override
  State<ParticleDriftOverlay> createState() => _ParticleDriftOverlayState();
}

class _ParticleDriftOverlayState extends State<ParticleDriftOverlay>
    with SingleTickerProviderStateMixin {
  late final AnimationController _c;
  final List<_Particle> _parts = List.generate(32, (i) => _Particle.random());
  @override
  void initState() {
    super.initState();
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        return CustomPaint(
          painter: _ParticlePainter(_parts, _c.value, widget.color),
        );
      },
    );
  }
...
class _Particle {
  ...
  _Particle.random() {
    ...
    x = rnd.nextDouble();
    y = rnd.nextDouble();
    r = 1 + rnd.nextDouble() * 2.5;
    s = 0.2 + rnd.nextDouble() * 0.8;
  }
}
...
class _ParticlePainter extends CustomPainter {
  ...
  final double t;
  final Color c;
  _ParticlePainter(this.parts, this.t, this.c);
  @override
  void paint(Canvas canvas, Size size) {
