...
class RatingService {
  ...
  static const _kLastSessionCountPrompted =
      'rating_last_session_count_prompted';
  ...
  static const int minShortSessionSeconds =
      180; // avoid prompting right after a very short session
  static const int rePromptAfterDaysIfPatch =
      30; // allow patch-level re-prompt after this many days if status == later
  ...
  Future<void> registerSession({
    required int lastSessionDurationSeconds,
  }) async {
    ...
    try {
      ...
      if (context.mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(t.ratingThankYou)));
      }
    } catch (_) {
      await _openStoreListing();
    }
  ...
  Future<void> maybePrompt(
    ...
    DebugLog.d(
      '[Rating] Evaluating prompt: status=$status sessions=$totalSessions',
    );
    ...
    DateTime? lastPrompt =
        lastPromptStr != null ? DateTime.tryParse(lastPromptStr) : null;
    ...
    final allowByVersion = _allowVersionRePrompt(
      status,
      lastVersionPrompted,
      currentVersion,
      lastPrompt,
    ...
    DebugLog.d(
      '[Rating] Version gate passed (current=$currentVersion last=$lastVersionPrompted)',
    );
  ...
  double _probability(int sessions) {
    ...
    return (baseProbability + extra * probabilitySlope).clamp(
      baseProbability,
      maxProbability,
    );
  }

  bool _allowVersionRePrompt(
    String status,
    String? lastVersion,
  ...
  Future<void> _showPrePromptSheet(
    BuildContext context,
    String version,
    int totalSessions,
  ) async {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () => Navigator.of(context).pop('later'),
                      child: Text(t.ratingPromptLater),
                    ),
                    TextButton(
                      onPressed: () => Navigator.of(context).pop('no'),
                      child: Text(t.ratingPromptNoThanks),
                    ),
  ...
  Future<void> _recordPrompt(
    ...
    DebugLog.d(
      '[Rating] Recorded prompt: version=$version sessions=$totalSessions status=${status ?? '(unchanged)'} times=${times + 1}',
    );
  }
