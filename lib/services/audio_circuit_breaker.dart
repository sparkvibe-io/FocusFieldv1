...
class AudioCircuitBreaker {
  ...
  static const Duration _immediateBlockTimeout = Duration(seconds: 5); // Immediate block after any audio activity
  ...
  bool canExecute() {
    ...
    if (_lastAudioActivity != null && !_immediateBlockActive) {
      ...
      if (timeSinceActivity.compareTo(_immediateBlockTimeout) < 0) {
        if (!kReleaseMode) {
          print('DEBUG: Audio access blocked by immediate protection (${_immediateBlockTimeout.inSeconds - timeSinceActivity.inSeconds}s remaining)');
        }
        return false;
      }
    }
    ...
    if (_flutterState == FlutterEngineState.crashed) {
      if (_lastFlutterError != null && 
          now.difference(_lastFlutterError!).compareTo(_flutterRecoveryTimeout) > 0) {
        _flutterState = FlutterEngineState.unstable;
        if (!kReleaseMode) print('DEBUG: Flutter engine: CRASHED -> UNSTABLE (recovery timeout)');
      } else {
        return false;
      }
    }
    ...
    if (_bufferState == AudioBufferState.crashed) {
      if (_lastBufferError != null && 
          now.difference(_lastBufferError!).compareTo(_bufferRecoveryTimeout) > 0) {
        _bufferState = AudioBufferState.unstable;
        if (!kReleaseMode) print('DEBUG: Audio buffer: CRASHED -> UNSTABLE (recovery timeout)');
      } else {
        return false;
      }
    }
  ...
  void recordSuccess() {
    ...
    if (_bufferState == AudioBufferState.unstable) {
      ...
      _bufferErrorCount = 0;
      if (!kReleaseMode) print('DEBUG: Audio buffer: UNSTABLE -> STABLE (recovered)');
    }
    ...
    if (_flutterState == FlutterEngineState.unstable) {
      ...
      if (!kReleaseMode) print('DEBUG: Flutter engine: UNSTABLE -> STABLE (recovered)');
    }
  }
  ...
  void recordBufferError(String errorType) {
    ...
    if (!kReleaseMode) {
      print('DEBUG: Audio buffer error: $errorType (count: $_bufferErrorCount)');
    }
    ...
    if (errorType.contains('releaseBuffer') || 
      ...
      if (_bufferErrorCount >= 2) {
        ...
        if (!kReleaseMode) print('DEBUG: Audio buffer: STABLE -> CRASHED (buffer sync issues)');
      } else {
        _bufferState = AudioBufferState.unstable;
        if (!kReleaseMode) print('DEBUG: Audio buffer: STABLE -> UNSTABLE (buffer issues detected)');
      }
    }
  }
  ...
  void recordFlutterError(String errorType) {
    ...
    if (!kReleaseMode) {
      print('DEBUG: Flutter engine error: $errorType (count: $_flutterErrorCount)');
    }
    ...
    if (errorType.contains('nativeDispatchPlatformMessage') || 
      ...
      if (_flutterErrorCount >= 1) { // Lower threshold for Flutter crashes
        _flutterState = FlutterEngineState.crashed;
        if (!kReleaseMode) print('DEBUG: Flutter engine: STABLE -> CRASHED (communication issues)');
      } else {
        _flutterState = FlutterEngineState.unstable;
        if (!kReleaseMode) print('DEBUG: Flutter engine: STABLE -> UNSTABLE (communication issues detected)');
      }
    }
  }
  ...
  void recordAudioActivity() {
    ...
    if (!kReleaseMode) {
      print('DEBUG: Audio activity detected - immediate protection activated for ${_immediateBlockTimeout.inSeconds}s');
    }
...
class SafeAudioExecutor {
  ...
  Future<T?> execute<T>(
    ...
    if (!_circuitBreaker.canExecute()) {
      if (!kReleaseMode) {
        print('DEBUG: SafeAudioExecutor: $operation blocked by circuit breaker (state: ${_circuitBreaker._state}, buffer: ${_circuitBreaker._bufferState}, flutter: ${_circuitBreaker._flutterState})');
      }
      return null;
    }
